...
def strip_package_version(raw: str) -> str:
    ...
    Примеры:
      ...
      "2019-12-20"         → "2019-12-20"  (date-версии сохраняются)
    """
    if not raw:
        return ""
    v = raw.strip()
    # убрать эпоху (число + двоеточие в начале: "1:2.3.4" → "2.3.4")
    v = re.sub(r'^\d+:', '', v)
    # убрать тильда-суффикс (~dfsg, ~rc1, ~bpo и т.д.)
    v = v.split('~')[0]
    # убрать + суффикс (+dfsg, +b1 и т.д.)
    v = v.split('+')[0]
    # убрать пакетную ревизию через дефис — только если часть после дефиса
    # содержит буквы (1ubuntu2, 1.el9, rc1).
    # Сохраняем date-версии типа "2019-12-20" (все части — только цифры).
    parts = v.split('-')
    clean = [parts[0]]
    for part in parts[1:]:
        if re.search(r'[a-zA-Z]', part):
            break  # это ревизия пакетного менеджера — дальше не берём
        clean.append(part)
